...
const Page = () => {
  ...
  const initiateOIDCMagicLink = async (email: string) => {
    try {
      ...
      const authUrl = `https://${baseURL}/oauth2/v1/authorize?response_type=code&client_id=${client_id}&redirect_uri=${redirect_uri}&scope=openid&code_challenge=${codeChallenge}&code_challenge_method=S256&state=${state}&flow_id=sign-in-magic-link&login_hint=${encodeURIComponent(email)}`;
      ...
      setErrorDescription("Failed to initiate the magic link flow. Please try again.");
    }
  };
  ...
  function generateCodeVerifier() {
    ...
    for (let i = 0; i < 64; i++) { // 64 characters for better compatibility
    ...
    <div className="flex flex-col items-center justify-center min-h-screen">
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4 max-w-md">
          <h3 className="font-bold">{error.replace(/_/g, ' ').toUpperCase()}</h3>
      ...
      <Descope
        ...
        onSuccess={(e: any) => {
          ...
          if (userEmail) {
            ...
            setErrorDescription("User email not found in authentication response. Please try again.");
          }
        }}
        onError={(e: any) => {
          console.log("Could not log in!", e);
          setError("authentication_failed");
          setErrorDescription(e.error?.message || "Authentication failed. Please try again.");
        }}
